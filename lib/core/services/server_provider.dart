...
import 'package:swiss_gold/core/services/spotrate_service.dart';
...
class GoldRateProvider extends ChangeNotifier {
  ...
  // Get the display price (asking price if available, fallback to bid)
  String getDisplayPrice() {
    final askingPrice = calculateAskingPrice();
    if (askingPrice != null) {
      return askingPrice.toStringAsFixed(2);
    }
    
    // Fallback to bid price if asking price can't be calculated
    final bid = _goldData?['bid'];
    if (bid != null) {
      return (bid as num).toStringAsFixed(2);
    }
